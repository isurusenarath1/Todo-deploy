...
import { updateTodoStatus, deleteTodo, permanentlyDeleteTodo } from '../api/todoApi';
...
function TodoItem({ todo, onUpdate }) {
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      setIsLoading(true);
      setActionType(newStatus === 'completed' ? 'completing' : 
                   newStatus === 'active' && todo.status === 'completed' ? 'reactivating' : 'restoring');
  ...
  const handlePermanentDelete = async () => {
    if (window.confirm('Are you sure you want to permanently delete this task? This action cannot be undone.')) {
      try {
        setIsLoading(true);
        setActionType('permanentDeleting');
          ...
          <div className="flex space-x-2">
            ...
            {todo.status === 'completed' && (
              <button
                ...
                disabled={isLoading}
                className="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600 text-sm flex items-center disabled:opacity-70"
              >
                {isLoading && actionType === 'reactivating' ? <LoadingSpinner /> : <ReactivateIcon />}
                Reactivate
              </button>
            )}
            ...
            {todo.status === 'deleted' && (
              <>
                <button
                  onClick={() => handleStatusChange('active')}
                  disabled={isLoading}
                  className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-sm mr-2 flex items-center disabled:opacity-70"
                >
                  {isLoading && actionType === 'restoring' ? <LoadingSpinner /> : <RestoreIcon />}
                  Restore
                </button>
                <button
                  onClick={handlePermanentDelete}
                  disabled={isLoading}
                  className="px-3 py-1 bg-red-700 text-white rounded hover:bg-red-800 text-sm flex items-center disabled:opacity-70"
                >
                  {isLoading && actionType === 'permanentDeleting' ? <LoadingSpinner /> : <DeleteIcon />}
                  Delete Forever
                </button>
              </>
            )}
          ...
          <div className="flex flex-wrap gap-2 mt-2 justify-end">
            ...
            {todo.status === 'completed' && (
              <button
                onClick={() => handleStatusChange('active')}
                disabled={isLoading}
                className="px-2 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600 text-xs flex items-center disabled:opacity-70"
              >
                {isLoading && actionType === 'reactivating' ? <LoadingSpinner /> : <ReactivateIcon />}
                Reactivate
              </button>
            ...
            {todo.status === 'deleted' && (
              <>
                ...
                <button
                  onClick={handlePermanentDelete}
                  disabled={isLoading}
                  className="px-2 py-1 bg-red-700 text-white rounded hover:bg-red-800 text-xs flex items-center disabled:opacity-70"
                >
                  {isLoading && actionType === 'permanentDeleting' ? <LoadingSpinner /> : <DeleteIcon />}
                  Delete Forever
                </button>
